...
character ::
  Parser Char
character = P (\t -> case t of
                      (x :. xs) -> Result xs x
                      Nil -> ErrorResult UnexpectedEof) -- TODO: error case!
...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f (P p) = P (\x -> case p x of
...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser f (P p) = P (\x -> case p x of
                               (ErrorResult e) -> ErrorResult e
...
list1 p = flbindParser p (\x ->
          flbindParser (list p) (\x' ->
            valueParser (x :. x')))
  -- run parser, bind to a parser that accepts success and runs it into list
  -- then append.
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy p = P (\t -> case t of
                      (x:.xs) -> if p x then Result xs x else ErrorResult Failed
...
surnameParser = flbindParser upper (\x ->
                  flbindParser (thisMany 5 lower) (\xs ->
                    flbindParser (list lower) (\xs' ->
                      valueParser (x :. (xs ++ xs')))))
...
-- note: copied from answers, too lazy.
...
instance Functor Parser where
  (<$>) ::
    ...
    -> Parser a
    -> Parser b
  f <$> p  = flbindParser p (\x -> valueParser (f x))
...
instance Apply Parser where
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  f <*> p = flbindParser f (\x ->
            flbindParser p (\y -> valueParser (x y)))
    -- get function from f, run p, then put output into f's function.
    -- error "todo: Course.Parser (<*>)#instance Parser"
