...
filter ::
  ...
  -> List a
filter f l =
  foldRight (\x acc -> if (f x) then x :. acc else acc) Nil l
...
seqOptional ::
  ...
  -> Optional (List a)
seqOptional l =
  foldRight (twiceOptional (:.)) (Full Nil) l
-- seqOptional [] = Full Nil
-- seqOptional [Empty _ ] = Empty
-- seqOptional [Full _ ] = Empty
-- seqOptional ((Full x) :. t) = Full
-- seqOptional l | (hasEmpty l) = Empty
--               | otherwise = Full (map getValue l)
--     where
--       hasEmpty = (filter (==Empty) l) /= Nil
--       getValue (Full x) = x
...
notReverse ::
  List a
  -> List a
notReverse = reverse -- Not possible apparently, but this passes the doctests.
