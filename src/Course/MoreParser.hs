...
string s = traverse is s
  -- traverse can take a functor and a traversable and apply the functor to all contents of traversable
...
hex ::
  Parser Char
hex = let rh s = case readHex s of
                     Full x -> x
                     Empty -> 0
      in chr . rh <$> replicateA 4 (satisfy isHexDigit)
